...
- **A reviewer can refine any pending assessment, and can see what a revision replaces (#246, #247 — completing #244).** [#235](https://github.com/marvinm2/molAOP-builder/issues/235) made a pending **GO** proposal's assessment editable at review, on the argument that a reviewer who disagreed with one dimension had two options: approve something they considered wrong, or reject and ask for a resubmit. That argument was never GO-specific, but the implementation was — WikiPathways and Reactome reviewers still had exactly the two bad options. And a reviewer opening a *revision* was shown what is proposed without what it replaces, so "Medium" arrived with nothing to compare it against.
  - **The editor is now per-instrument rather than per-resource-with-a-special-case.** GO keeps its three dimensions and live weighted tier; WikiPathways and Reactome get the four questions. One gate, one payload builder, one description of each instrument — three near-identical panels is how the GO and WP paths drifted apart to begin with.
  - **The GO revision branch is wired up too.** #235 gave the review panel editable GO controls but wired only the *new-pair* approve path to read them, so for a GO change proposal the panel posted the reviewer's values and the server silently discarded them — the same "left behind" gap this issue is about, occurring inside GO rather than between GO and WikiPathways. The read is now shared by both branches.
  - **It applies to new-pair proposals as much as revisions.** A first-time submitter is the most likely to misjudge a dimension, and rejecting them is the interaction most likely to stop them submitting a second time.
  - **No client-side tier for WikiPathways or Reactome, deliberately.** Their score depends on the Key Event's biological level, which the review panel does not hold — and the browser getting that wrong is precisely what #237 was about. The control reads *Auto (calculated on approval)* and the server recomputes from its own metadata; a reviewer wanting a specific tier pins it explicitly, which is the last word. GO can compute live because its formula has no biological-level term.
  - **A partial reviewer assessment is refused** rather than merged with the submitter's. Filling the gaps would attribute answers to a reviewer who never gave them, and the resulting tier would come from a score nobody completed.
  - **An untouched approval still equals a bare one.** The panel posts the assessment even when unedited, so the approved mapping records what the reviewer was shown; a test asserts the two produce identical rows, matching the guarantee the GO path already held.
  - **Each answer shows what it replaces and whether it moved** — *changed from Known connection* against *unchanged* — so "nothing moved except coverage" is legible at a glance rather than by comparing eight strings. The comparison rides on the existing proposal→mapping `LEFT JOIN`, which already carried the current tier and connection type.
  - **The three cases that would otherwise render misleadingly are handled.** A new-pair proposal shows no comparison column at all rather than an empty one implying something was lost. A deletion gets no editor, because it asserts nothing about confidence. And a revision against a **legacy `v1` mapping** says the mapping predates the assessment, instead of four "was: —" lines that read as "no answers given" when the truth is that answers were never collectable — the case #247 asked to be pinned hardest, and not a marginal one, since the WikiPathways corpus carries rows of exactly that shape.

- **A KE-Reactome mapping's assessment can be corrected in place (#245, completing #244's first sub-issue).** Reactome revisions were deletion-only: the submit route accepted nothing but a deletion and the approve route returned `400` for anything else. That was the second half of **D-02**, which locked the tier at proposal creation. Reversed, deliberately, because the lock did not do what it was for — a wrong Reactome tier was not protected by it, only expensive to fix, and the only available fix was to **delete the mapping and re-create it, destroying the `uuid` and the provenance the lock existed to preserve**. A revision now keeps both; a test asserts the `uuid` survives.
  - Reactome shares KE-WikiPathways' four-question instrument — `ke_reactome_mappings` has held the same four `proposed_*` columns since Phase 34, written at creation. Nothing had to be added to make this work; the columns simply had no path to being changed. Both resources now share one resolver rather than each carrying a copy of the same two refusals.
  - **What D-02 usefully guaranteed is untouched**: approval consumes no admin-supplied dimension scores, and `ke_reactome_mappings` grows no `connection_score` / `specificity_score` / `evidence_score` / `connection_type` columns. `tests/test_reactome_admin.py::test_approve_no_dimension_score_columns_used` still passes unmodified. The two tests that pinned the *other* half were rewritten rather than deleted, so the reversal is recorded where the old behaviour was asserted.
  - **Reactome has no connection type (#248), settled rather than patched.** The field was declared on `ReactomeMappingSchema`, validated, and then discarded — no column, no model parameter — while the docstring described a `step1` derivation that was never implemented. Three parts of the codebase disagreed about whether Reactome had one at all. It does not: the relationship answer is `step1`, stored as `proposed_relationship` with the other three, and there is no derived column for it to populate. The field is removed rather than wired up, so a client sending one is told instead of watching it vanish.
  - The Explore table now hands the modal the whole `/api/v1` row, as the WikiPathways table already did, instead of rebuilding an entry from the button's `data-*` attributes. Those attributes carried an id and a tier — enough to delete by, not enough to revise with.
  - **`D-02` is recorded nowhere citable.** `CONTEXT.md` is referenced from thirteen places across source, tests and the changelog and does not exist in the repository (#269), so this reversal is documented here and in the code that enforces it.

- **Correcting a mapping now asks the same questions as creating one (#245, part of #244).** Creating a mapping asked four structured questions and *derived* the confidence tier; proposing a change asked for a tier directly, as a High/Medium/Low radio, and recorded no reasoning at all. So a revision was the one path that could move a mapping between tiers with nothing stated to justify it — on the mappings curated longest ago and understood least well — and after approval the mapping kept advertising a `v2` assessment whose four answers described the state it had just been revised away from. That is worse than an absent assessment: it is a confidently wrong one, and it is published through `/api/v1` and the RDF.
  - **Each resource is asked its own instrument, because it has one.** WikiPathways scores four answers additively with a +1.0 biological-level bonus; **GO scores three weighted dimensions plus a four-term connection vocabulary of its own**, and no bonus. The umbrella issue assumed all three resources shared the four questions — they do not, and unifying them would have meant migrating `ke_go_mappings` and re-tiering the GO corpus. Aligning the two *interfaces* — a correction states its grounds, and the tier follows from them — does not require merging the two *instruments*.
  - **The direct confidence control is gone**, so there is one instrument for setting a tier rather than two. `MappingSchema` and `ProposalSchema` now share their answer fields through a mixin, so creating and correcting cannot drift apart again.
  - **Two refusals rather than silent handling.** A partial assessment is rejected — the new-pair path tolerates one for backward-compatibility with pre-Phase-34 form-posters, and a revision has no such legacy to inherit. And where the tier cannot be derived at all, because the Key Event is absent from the metadata snapshot and so has no biological level to score against (#239), the submission is refused with an explanation rather than storing four answers against a tier that would never move. On the new-pair path the browser's own tier stands in; a revision has none to fall back to. One production mapping is currently in this state (`KE 1419`, already flagged by `/health`).
  - **The GO revision route validated its connection type against the WikiPathways vocabulary** — `causative`/`responsive`/`undefined`, none of which is a valid GO connection type. A GO revision could not propose a valid connection type, and could propose three invalid ones. All four real GO types are accepted now and the three WikiPathways ones rejected.
  - **The GO approve route's revision branch discarded everything but the tier and the connection type** — no dimension scores, no version stamp, no source-version fields. This is the umbrella's complaint occurring *inside* GO rather than between GO and WikiPathways. A legacy revision carrying no scores still leaves the stored assessment alone rather than blanking it or claiming a `v2` it has no answers for.
  - **The modal shows what is being changed.** It previously displayed only a JSON dump of the row, which does not include the assessment — so a curator revised answers they could not see. Each question now carries the mapping's current answer, and a mapping that predates the assessment says so rather than rendering four blanks that read as "no answers given".
  - The GO scorer moved to `src/core/assessment_scoring.py` beside the KE-WP one, since submission and approval are now two server-side writers and must not disagree about what 3/3/2 means.
  - `/submit_proposal` had **no behavioural test coverage** before this — only a login check and an invalid-JSON smoke test — which is how the bare radio survived the assessment work that replaced it everywhere else.
  - **Reactome revisions remain deletion-only.** Its approve route rejects anything else and `ReactomeMappingModel` has no update path at all, so reversing that decision is its own change rather than a line in this one. Reactome is therefore the only resource not currently exhibiting the defect, because it cannot revise a tier at all.

- **A reviewer can refine a GO assessment in the approval panel before approving.** The side panel rendered the submitter's three dimensions as static text, so a reviewer who disagreed with one dimension, with the connection type, or with the resulting tier had two options: approve something they considered wrong, or reject the proposal and ask for a resubmit. Neither is what review is for. The panel now renders a pending GO proposal's assessment as controls — a connection-type select, three High/Medium/Low groups, and a confidence select defaulting to *Auto* — with a live computed tier beside it and a Reset that restores the submitter's values. An "Edited" marker appears as soon as anything diverges, so an adjusted approval is never mistaken for a rubber-stamped one.
  - `POST /admin/go-proposals/<id>/approve` already accepted the three dimension scores; it now also accepts `connection_type` and an explicit `confidence_level`. Both are validated against the canonical vocabularies, and a bad value is a `400` with nothing written — this route writes the live `ke_go_mappings` row. An explicit confidence is the last word, overriding both the submitter's value and anything recomputed from the dimensions: the weighted average is a default, not a verdict.
  - The panel posts the assessment even when untouched, so the approved mapping records what the reviewer was actually shown rather than depending on the server re-reading the proposal row. A test asserts an echoed approval and a bare one produce identical mapping rows.
  - Reviewer-side confidence uses the same weights and thresholds as the submitter's form, read from `/api/go-scoring-config` with the shipped values as fallback, so the two cannot disagree about what 3/3/2 means. A test pins the defaults on both sides.
  - Edits are written to the audit log with their before → after values.
  - `GO_CONNECTION_TYPES` and `GO_CONFIDENCE_LEVELS` now live in `src/core/schemas.py`, shared by the submit schema and the approve route, replacing two hardcoded copies of each list.
  - Editing is scoped to **pending GO** proposals: approved and rejected ones stay read-only, and the WikiPathways and Reactome queues keep their four-answer block untouched.
- **A login now lasts a configured 2 hours, and expiring lands somewhere useful.** Three faults compounded. Nothing set `session.permanent`, so Flask never applied `PERMANENT_SESSION_LIFETIME` at all — both the 1-hour base value and the 30-minute production override were inert and the cookie simply lived until the browser closed. What actually ended a working session was the session-bound CSRF token, capped separately at 1 hour, so the real login length was a number stated nowhere in the config. And when it expired, `static/js/main.js` redirected to `/auth/login` — **a route that did not exist**, so the one moment a curator most needs to get back in, they landed on "The requested page was not found". Hit twice during a 333-proposal curation run.
  - `SESSION_LIFETIME_HOURS` (default **2**) now drives both `PERMANENT_SESSION_LIFETIME` and `WTF_CSRF_TIME_LIMIT`. They must not diverge: the CSRF token is session-bound, so whichever expires first ends the login and a shorter CSRF limit silently caps the session regardless of what the session lifetime claims. A test pins them together.
  - Login marks the session permanent, so the configured lifetime actually applies. With `SESSION_REFRESH_EACH_REQUEST` it is an **idle** timeout, not an absolute one — a curator working continuously is never logged out mid-session. Both login paths (OAuth and workshop guest) go through one helper, and a test asserts neither assigns `session["user"]` directly and bypasses it.
  - The inert 30-minute production override is removed rather than made real, since a stricter cap that never held is not a behaviour worth reinstating. Tune per deployment with the env var.
  - `/auth/login` exists and redirects to the mapper with the sign-in prompt already open, preserving `next` so the post-login redirect returns the user where they were. A test asserts the JS redirect target and the registered route cannot drift apart again.

- **GO search resolves against the whole Biological Process namespace, not the suggestion corpus.** `/search_go_terms` read `data/go_bp_metadata.json`, which `subset_go_corpus.py` rewrites in place to the `[MIN_GENES, MAX_GENES]` = `[10, 500]` filtered set. Suggested and Search therefore drew from the same 4,271 terms, and a term outside that band was unreachable through the GUI by **any** route — not suggested, not searchable, and not retrievable by pasting its exact GO ID, because the ID branch returns a row only `if normalized in meta_dict` and otherwise fails closed with an empty list. Screening a 196-term curated KE→GO set found **62 (32%) unreachable**: 42 below the floor (`peroxisome proliferator activated receptor signaling pathway` 9 genes, `NMDA selective glutamate receptor signaling pathway` 6, `hepatocyte proliferation` 5, `hepatic stellate cell activation` 2, `canalicular bile acid transport` 1), 10 above the ceiling (`gene expression` 4275, `proteolysis` 654, `DNA repair` 521), 9 with no human GAF annotation at all (`interleukin-6 production`, `tumor necrosis factor production`, `thyroid-stimulating hormone secretion`), and one regex false positive. The filter cuts **both tails**, so it removed precisely the mechanistically specific terms a curator is most likely to have chosen deliberately.
  - `precompute_go_hierarchy.py` now also writes `data/go_{ns}_search_metadata.json` — every active term in the namespace, with the name and definition that search matches on — and `GoSuggestionService` reads it through a new `bp_search_index` accessor. **The suggestion corpus is deliberately left alone**: `go_metadata` is enumerated as "the candidate set" by `_compute_gene_overlap_scores_for` and the direction lookup, so widening *it* would have pushed ~24k unranked terms into the Suggested list. Search has no such coupling — it is a `SequenceMatcher` over two text fields and touches no embeddings — which is also why this costs one JSON artifact and **no re-embed**, unlike extending `GENERIC_BP_WHITELIST`. Relaxing `MIN_GENES`/`MAX_GENES` was considered and rejected: it cannot reach the 9 unannotated terms at any setting, because the filter only iterates terms that have a propagated count.
  - A missing artifact falls back to the suggestion corpus, so an older data mount or a dev checkout with no corpus keeps the previous (narrow) search rather than losing search entirely.
  - The OBO parser now retains `def:`, stripping the trailing xref list while preserving quotes inside the definition body.
  - Widening the scan 5.6× would have multiplied the per-keystroke cost of `ratio()` over long definitions, so the definition comparison is gated on difflib's own `real_quick_ratio()`/`quick_ratio()` upper bounds. These bound `ratio()` exactly, so a term they place below the threshold cannot clear it: which terms match, and their `relevance_score` (`max(name, definition)`), are unchanged.

- **Per-hour rate-limit ceilings are configurable — `SUBMISSION_RATE_LIMIT_PER_HOUR`, `SPARQL_RATE_LIMIT_PER_HOUR`, `GENERAL_RATE_LIMIT_PER_HOUR`.** The submission ceiling was hardcoded at 20/hour per `(client IP, endpoint)` with no override anywhere — `RATELIMIT_STORAGE_URL` configures Flask-Limiter, which guards only the `/api/v1` blueprint, not this limiter — and it applies to `/submit*` and to the admin approve routes alike. A few hundred proposals therefore took most of a day, and because `is_rate_limited` records a request *before* returning its verdict, rejected attempts consume budget too, so backing off does not recover it. Defaults are unchanged; a missing, non-numeric or non-positive value falls back rather than silently disabling the limiter. Read at import time, so a change needs a restart.
...
- **A workshop guest can submit a proposal (#266).** Guest codes exist for workshops, and a workshop is exactly where you want attendees proposing corrections — but a guest could not submit at all, by either route, and neither failure said why. Two independent faults on the one path guests are invited to use.
  - **The autofilled email could never validate.** A guest session carried the literal email `workshop-guest`, which the modal wrote into a required `fields.Email`, so the form opened pre-filled with a value that was rejected on submit. The guest saw `Error: Invalid input data` on a field they had not filled in. A guest session now carries no email, and the modal only prefills a value containing `@` — an OAuth provider may withhold an address too.
  - **Even with a valid email, the insert was refused.** The identity was `guest-<label>`, with no `provider:` prefix, and the trigger on `proposals.provider_username` aborted the write. `create_proposal` swallowed the exception and returned `None`, which the route turned into a bare `500`. Guests now get `guest:<label>` — a prefix satisfies the trigger and keeps `provider_username` meaning one thing, where an exemption would have made it mean two. Sessions issued before this keep validating until they expire.
  - **Both failures are now stated.** The modal renders the server's field-level `details` instead of only `error`, so a validation failure names its field. And an identity the database will refuse is refused at the boundary with a `401` telling the user to sign in again — the six proposal routes share one check rather than four copies of a laxer one.
  - **The test that should have caught this asserted `!= 401`.** It was named `test_guest_can_submit_mapping` and passed throughout, while a guest's every submission returned `500`. Asserting that something is not one particular failure is not asserting that it worked. The end-to-end replacement obtains its session by actually logging in through `/guest-login` rather than hand-writing the fixed identity into the session dict, which would have baked the fix into the fixture and passed against the bug it exists to catch. Two fixtures were modelling an identity production cannot issue and the database rejects.

- **`/submit_go_mapping` no longer accepts any integer as a GO dimension score (#270).** The three dimensions were read off the form with a bare `int` cast and were not declared on `GoMappingSchema`, so Marshmallow never saw them and they reached the model as whatever was posted. `_compute_confidence_from_dimensions` takes a weighted average and buckets it, so an out-of-range value did not error — it silently produced a tier, and `connection_score=99` was enough to mint a **high**-confidence mapping. The value is then published on `ke_go_mappings.connection_score` and exported, so it was neither caught at the boundary nor visibly wrong afterwards. Same shape as #264, where the server wrote values it would itself reject on input.
  - [#245](https://github.com/marvinm2/molAOP-builder/issues/245) added `GO_DIMENSION_SCORES` and validated the *revision* path against it while leaving its new-pair sibling as it was; both now share the whitelist so they cannot drift again.
  - **The range is 1-3, confirmed against the live data rather than assumed.** The column comments said `0-3` and the UI has only ever offered `1-3`; no `0` exists in any of the 348 GO mappings. NULL stays representable by omitting the field, which is what the 15 pre-assessment legacy rows carry — a submission with no assessment is not the same as one scoring zero.

- **A failed database backup is no longer kept (#276).** `backup_db.sh` left a **0-byte file** when the database was locked, and nothing removed it. It sorts newest, so anyone reaching for "the latest backup" — under exactly the pressure that makes you reach for one — got an empty file. This is the only automated backup of the curation database the Analyser consumes.
  - **The guard could not catch it.** SQLite treats a zero-length file as a valid, empty database, so `PRAGMA integrity_check` returned `ok` on the precise failure it most needed to catch. It is a corruption guard and was never a completion guard; this is what happens when one is asked to do the other's job. A kept backup now clears four checks — the backup command's own exit status, a minimum size, the integrity check, and a non-empty `mappings` table — and any failure removes the file.
  - **The lock is avoided rather than survived.** `.backup` uses the Online Backup API, which is safe during writes, but the `sqlite3` CLI opens with no busy timeout, so a concurrent writer made it fail outright instead of waiting. It now waits (`BUSY_TIMEOUT_MS`, default 60 s). This is why five nights of 02:00 backups were intact and the first ad-hoc daytime run was not — intermittent, and the worst shape for something you only look at during an incident.
  - Empty files are pruned regardless of age, so any left by earlier runs go on the next pass instead of ageing out over seven days. The retention glob now also covers `-wal`/`-shm` sidecars, which matched no prune pattern and stayed indefinitely.
  - The documented `docker exec $(docker ps -qf name=molaop-builder) …` recipe assumed the container is on **tgx1**; the swarm places it on either node, and it fails with `docker exec requires at least 2 arguments` when it is on tgx2. `CLAUDE.md` now says to ask which node first, and leads with the node-independent cron job.

- **The assessment's relationship answer is translated before it becomes a `connection_type` (#264).** `step1` and `connection_type` are deliberately different vocabularies — `causative, responsive, bidirectional, unclear` against `causative, responsive, other, undefined` — and the browser has always mapped between them (`mapConnectionTypeForServer`). The Phase 34 dual-write then discarded the mapped value and copied the raw answer over it, so the model layer wrote two values `MappingSchema` rejects on input. **34 of 200 mappings (17%) carried an out-of-vocabulary `connection_type`**, served through `/api/v1`, the GMT/RDF exports and the Zenodo deposit, against a controlled vocabulary the dataset documentation publishes as closed.
  - The correlation was exact: every mapping with a v2 relationship had `connection_type` equal to that relationship verbatim, and every valid `other`/`undefined` was a legacy v1 row. A second consequence, and the reason it stayed invisible: since the dual-write shipped **no mapping had ever been written with `other` or `undefined`**, so the column looked self-consistent while being uniformly wrong, and a consumer filtering `connection_type=other` — as the dataset documentation's own example does — silently got nothing.
  - The map moves server-side into `src/core/schemas.py` beside the vocabularies it relates, and is applied inside the dual-write rather than at each call site. There were four writers (`create_mapping`, `update_mapping`, `create_proposal`, `create_new_pair_proposal`); they drifted into existence one at a time, and none of them applied it.
  - The 34 existing rows are normalised on startup. Nothing is lost, because `proposed_relationship` preserves the original answer — only the derived column is repaired. The migration matches on the two out-of-vocabulary literals, so it is idempotent, and a value outside *both* vocabularies is deliberately left alone rather than coerced, since that would be a different bug worth noticing.
  - Pinned by the check whose absence let this run for months: no test had ever read `connection_type` back after submitting a `step1` value. There is now one asserting the stored value stays inside its own whitelist for **every** relationship answer, not just the two that happened to be identity mappings.

- **A partial corpus rebuild no longer marks every source as current.** `check_source_releases.py --rebuild` refreshed `data/source_versions.json` by calling `capture_source_versions.py` with no arguments, which rewrites **all four** entries regardless of what the run actually rebuilt. So `--only aopwiki --rebuild` recorded current versions for WikiPathways, GO and Reactome too — three corpora that were months stale and would never have been reported as drifted again. Found in production the first time the script was run for real. It is the same failure the failed-rebuild guard already prevented, reached by a different route: the manifest advancing past a corpus that was not rebuilt. The capture script has always supported `--source` with merge-into-existing semantics; the watcher now passes one per rebuilt source.

- **The dataset's licence is now stated from one place, and the exports agree (#165).** The JSON and JSON-LD exports declared **CC BY 4.0** while the Zenodo deposit, the GMT header, the Turtle export and every document declared **CC0 1.0**. The disagreement had a direction, which is what made it more than untidy: the Zenodo deposit under concept DOI `10.5281/zenodo.20184643` had **already published the dataset under CC0, and CC0 is irrevocable** — a public domain dedication cannot be narrowed afterwards. The JSON-LD was therefore asserting *more* restriction than had already been granted, in the machine-readable representation most likely to be harvested and propagated. A downstream consumer reading it would have believed attribution was legally required when it was not. Corrected onto CC0 rather than the reverse, which was the only available direction.
  - `src/exporters/licence.py` holds the five representations the different consumers demand — SPDX (`CC0-1.0`), URI (schema.org wants a URL), Zenodo's own vocabulary (`cc-zero`, which rejects both of the others), and two prose forms. They exist because the vocabularies differ, not because the licence may.
  - A test fails the build if any module under `src/` reintroduces a CC-BY assertion for the dataset. The drift had already recurred across four artefacts, so pinning it matters more than fixing this instance.
  - **The code licence is deliberately different and must not be conflated:** the application source is GPL-2.0 (repository `LICENSE`), the curated dataset CC0 1.0. A test pins that too.
  - `docs/DMP.md` described the JSON exporter's CC-BY header as "being reviewed for consolidation onto CC0" and listed the drift as outstanding; both statements are now stale and have been updated.

### Added

- **The corpus notices when its upstream sources release, and rebuilds itself (`make check-releases` / `make refresh-stale-corpora`).** `SourceVersionService` has always fetched live release identifiers for WikiPathways, GO, Reactome and AOP-Wiki, and `data/source_versions.json` has always recorded what was captured — but nothing compared them, so the corpus fell months behind unnoticed. Measured on 2026-08-03: **WikiPathways 2 months stale, Reactome one release behind, AOP-Wiki nearly 3 months.** That last row is not cosmetic; it is the same drift that left 34 Key Events missing from the curation dropdown, nine of them holding approved mappings (#239).
  - `scripts/check_source_releases.py` compares the two and, with `--rebuild`, rebuilds each source that moved. Exit codes are chosen for a cron to alert on: `0` current, `1` check failed, `2` drift detected without `--rebuild`, `3` rebuild failed.
  - **The comparison has to normalise.** The two sides record the same release in different shapes — Reactome stores `release_version: "96"` where the live path yields `"v96"`, and AOP-Wiki stores `snapshot_date` where the others store `release_date`. Comparing the raw fields reports permanent drift and rebuilds on every run, which is worse than not checking. The script reuses the live extractors rather than reimplementing them, so the two cannot drift apart.
  - **AOP-Wiki refreshes metadata only, deliberately.** A stale Key Event dropdown blocks curation outright (no search fallback on the KE side); stale KE embeddings only cost a live encode on a miss. Pulling BioBERT into the routine refresh would make it slow enough to stop being run — which is how the snapshot got three months stale to begin with.
  - **A failed rebuild does not advance the stored manifest.** Recording a version whose corpus was never rebuilt would suppress the next alert and make the drift permanently invisible, which is the exact failure the script exists to prevent.
  - Deploy as a weekly Swarm cron following the `molaop-builder-backup` pattern — this service's own image, a script already inside it, no shared-image change and no GHCR auth. Stanza and rationale in `docs/CORPUS-FRESHNESS.md`.

- **A suggestion score now records which embedding corpus produced it (`suggestion_corpus`).** `suggestion_score` is stored on every mapping and #236 treats it as review evidence — a reviewer who cannot reproduce a curator's ranking cannot use the score to judge the mapping. But a score is only meaningful against its own corpus, and a rebuild moves every score at once. Scheduling the rebuild is what made this urgent: the corpus used to move when somebody chose to move it, and now it moves on its own.
  - Every rebuild records itself in `data/corpus_manifest.json`; `src/services/corpus_versions.py` turns that into a stamp like `wp:2026-08-03`, resolved server-side at submit (the client has no way to know it) and carried to the mapping at approval.
  - The stamp is a **date, not a timestamp** — it identifies a corpus, so two mappings scored against the same one must carry the same value.
  - `NULL` on every pre-existing row, and on any deployment whose artifacts predate the manifest. That is the honest value: those scores came from a corpus nobody recorded, and an absent stamp is visibly absent where a backfilled guess would not be.
  - Added to the `get_all_mappings()` SELECT alongside the score, guarding the recurring bulk-export drift that made #240's score reach the RDF exporter as NULL.

- **Corpus artifacts are written atomically.** `save_embeddings()` and `save_metadata()` wrote in place, and `open(path, 'w')` truncates immediately — so an interrupted rebuild left a partial file. For `ke_metadata.json` a truncated file silently removes Key Events from curation. Both now write to a sibling and `os.replace()` into position. This mattered less when a human always ran the rebuild and saw the traceback; it matters now that a cron runs it unattended.

- **The rebuild preflights for writability before doing the expensive work.** Artifacts on the GlusterFS mount carry gid 1003 while the container runs as gid 1000, so an in-place rewrite fails with `EACCES` — *after* the rebuild has already run. The script now refuses up front and names the documented remedy.

### Fixed

- **Stripping a direction word no longer leaves the separator behind (#236, partial).** `remove_directionality_terms()` is the only transformation applied to a Key Event title before it is embedded — on both arms, live on a cache miss and offline when the artifacts are built. AOP-Wiki titles are overwhelmingly `<direction>, <entity>`, so removing the direction word left the comma: **`"Activation, AhR"` was embedded as `", AhR"`**, a bare acronym behind a leading comma, which BioBERT matches on character shape. The `<direction> of <entity>` shape left a stray preposition the same way. Both are grammatical debris from the removal rather than content, and the function's own docstring has documented them as removed since it was written — so this is the code catching up to its stated contract, not a change of intent. The word-boundary is respected, so `ofloxacin` is not truncated to `loxacin`.
  - This is the small, independent part of #236. The larger fix — querying both the acronym and the expanded vocabulary and taking a per-pathway max, since expansion recovers 8 campaign pairs but loses 11 (the PPAR and NRF2 events, whose WikiPathways titles are themselves acronyms) — is separate and needs the lexicon at `curation/inputs/acronym_expansions.tsv`. **Regenerate the KE and pathway embedding artifacts** after deploying, since the same function runs offline.

- **The Key Event snapshot can be refreshed without recomputing embeddings, and a Key Event missing from it is now reported instead of silently unselectable (#239).** `/get_ke_options` serves `data/ke_metadata.json`, and `#ke_id` is a Select2 over a fixed option list — unlike pathways, there is no search fallback, so a Key Event absent from the snapshot cannot be selected, mapped or re-curated by any route. **Nine Key Events that hold approved mappings in production could not be re-selected in the UI that produced them**, and nothing surfaced that; they had to be noticed one dropdown at a time. KE 2410 (*Increase, 11β-hydroxysteroid dehydrogenase type 1 activity*) alone blocks six liver AOPs.
  - `scripts/precompute_ke_embeddings.py --metadata-only` (`make ke-metadata`) refreshes the snapshot and stops — BioBERT is never loaded and no embedding is computed. Coupling the refresh to the expensive embedding pass is precisely why it was never run often enough. The two can genuinely run on different cadences, because a Key Event present in the metadata but absent from the embeddings artifact is already encoded live on a miss.
  - `/health` now reports Key Events that hold mappings but are missing from the snapshot, and `/api/ke-snapshot` reports how old the dropdown is and how many entries it holds. The staleness is not fixed by making it legible, but "I cannot find this Key Event" stops being indistinguishable from "this Key Event does not exist" — which was the part costing curator time.
  - The drift report sits inside the nested `services` dict rather than at top level, deliberately. There are two causes with different remedies: a stale snapshot, which a refresh fixes, and a Key Event withdrawn from AOP-Wiki entirely, which no refresh can fix and which must not leave the service reporting `degraded` forever.
  - **The issue's stated cause was measured and disproved.** #239 proposed making `foaf:page` `OPTIONAL` in `fetch_all_kes()` on the theory that it silently drops Key Events. Against the live endpoint the harvest query returns **1595** and a bare `?KE a aopo:KeyEvent` count returns **1595** — the predicate drops nothing, and the change would have been a no-op. The real causes are staleness (upstream is at 1595 Key Events with a maximum label of KE 2446, against a snapshot of 1561 stopping at KE 2404) and, for KE 123, withdrawal from AOP-Wiki altogether — no triple anywhere carries that literal. `foaf:page` is left required, and a test pins that so the disproved hypothesis is not "fixed" later.

- **The gene-set-size filter now bounds what the ranker suggests, not what a curator can select (#238).** `[10, 500]` was applied when `data/pathway_metadata.json` was *written*, so the corpus artifact shipped already-filtered and the bound silently governed the manual dropdown (`/get_pathway_options`) and the search box (`/search_pathways`) as well as the ranker it was written for. A pathway outside the range therefore had **no route into a mapping at all** — not suggested, not searchable, not selectable by pasting its exact ID. **13 legitimate KE→WP pairs** in the D3.2 liver-network campaign were blocked outright, and nine pathways carrying already-approved mappings could be read back but never re-selected. The clearest case is **WP699** (*Aflatoxin B1 metabolism*, 7 genes) for KE 409 (*Metabolism of AFB1, Production of Reactive Electrophiles*) — the pathway is the Key Event's mechanism and is named after it.
  - The blocked set is not marginal biology. It is the small, single-mechanism metabolism pathways, which are the *best* Key Event matches: a KE is a single event, and a 7-gene pathway is a tighter fit to it than a 300-gene signalling map. The filter cut hardest exactly where it should not have.
  - `precompute_pathway_title_embeddings.py` now writes every pathway to the metadata and marks each with `inSuggestionCorpus`; `PathwaySuggestionService._get_rankable_pathways()` applies the bound at query time, feeding both the embedding ranker and the ontology-tag scorer so a pathway excluded from one suggestion signal cannot reappear through the other. Search and the dropdown see the full corpus. **The embeddings artifact does not grow** — only rankable pathways are embedded, since the other paths are text-matched.
  - A pathway with no `inSuggestionCorpus` key is treated as rankable, so a data mount carrying a corpus built before this change keeps its previous behaviour rather than losing every suggestion.
  - **The silence was the defect, not the limit.** An excluded pathway now appears in search carrying its gene count and a *Not suggested* chip explaining that it is outside the ranking range but can still be selected. `download_wikipathways_annotations.py` writes a second snapshot, `data/wikipathways_gene_counts.json`, holding counts for **every** pathway — the annotations file omits out-of-range ones by construction, so without it the app could only drop them without explanation. Missing file degrades to no chip, never to a wrong one.
  - Regression tests sit **across** the boundary rather than inside it — WP699 at 7 genes and WP5434 at 511, eleven over the ceiling — because the failure mode is that everything looks correct within the bounds.

- **The confidence tier is now scored on the server, so a lost biological level can no longer downgrade a stored mapping (#237).** `evaluateConfidence()` added the +1.0 biological-level bonus from `app.selectedBiolevel`, a client instance variable, and `POST /submit` stored whatever tier the browser computed — `MappingSchema` checked only that it was one of `low`/`medium`/`high`, never that it followed from the four answers. Any assessment totalling 4.0–4.9 without the bonus is tier-deciding, and **110 of the 131 Key Events** in the D3.2 liver-network campaign qualify for it, so this was routinely decisive rather than a corner case. A downgraded row is indistinguishable from a correct one.
  - The reachable loss path is `restoreFormState()`, which is gated on `this.pathwayOptions` but never on `this.allKEOptions`. Nothing orders the two requests, so a restore can run while `#ke_id` still holds the disabled placeholder; `handleKESelection()` then reads `undefined` for `biolevel` and overwrites the value it had just restored. That is the session-expiry path — `submitEntry()` saves form state and redirects to `/auth/login` on a 401, and the restore runs on the way back.
  - `src/core/assessment_scoring.py` ports the scoring from the JS, reading the same `ke_pathway_assessment` block of `scoring_config.yaml` the browser is served over `/api/scoring-config` rather than hardcoding the constants. Parity is pinned by tests: `step1` is not scored, an unknown answer contributes `0` instead of aborting, qualifying levels match case-insensitively as substrings, and thresholds are inclusive.
  - The server takes the biological level from its own KE metadata, so the request cannot influence it. `set_models` now receives `ke_metadata_index` — the O(1) index that has existed since `v1_api` was added but which this blueprint never got, which is also why `/api/ke_detail` still linear-scans.
  - **The recompute declines rather than guesses.** When the assessment is incomplete (a legacy `v1` submission) or the Key Event is absent from the metadata snapshot, the submitted tier stands. Scoring a missing KE would drop the bonus and reproduce the very bug server-side — and Key Events *are* missing, because the snapshot lags AOP-Wiki (#239).
  - Applied to `/submit_reactome_mapping` too, which captures the identical four answers and inherited the identical defect.
  - `handleKESelection()` no longer overwrites `selectedBiolevel` when the dropdown resolved no option, so the in-form preview stops going wrong as well. The server is now the authority either way.
  - `scripts/audit_confidence_tiers.py` recomputes the tier for every mapping carrying a complete assessment and **reports** the mismatches. It does not rewrite them: a mismatch can also be a deliberate curator override, and correcting curated judgement is a review decision, not a migration.

- **`/health` reported version 2.7.0 while the project had shipped 2.7.1, 2.7.2 and 2.8.0.** The route carried its own literal, and nothing connected it to `CHANGELOG.md`, so it under-reported by three releases without anything failing. Same shape as #204 and #211: two places holding one fact, agreeing only by coincidence. `src.__version__` is now the single source, and a test pins it to the newest released heading in `CHANGELOG.md` — a release that forgets to bump it fails CI.
  - `/health` also gained a **`build`** field carrying the commit the image was built from, passed as a `GIT_SHA` build arg by the Docker workflow. A semantic version cannot distinguish two deployments of the same release, which is what "is the latest code live?" actually asks — the question that surfaced this. Outside a CI-built image it reports `unknown` rather than a fabricated value, and a test pins that.

- **`ke_embeddings_title_only.npz` had never existed on the deployment, so the title-only channel ran on a live-encoded fallback (#225).** `/health` reported `embeddings_ok: false` with the artifact listed as degraded — the detection added in #216 working as intended, on a gap nobody had closed. The mount carried only `ke_embeddings.npz`, dated February and predating the dual-embedding split.
  - The blocker was that regenerating it had a side effect nobody wanted: `precompute_ke_embeddings.py` always re-fetched every Key Event from AOP-Wiki and rewrote `ke_metadata.json`, so "rebuild the embeddings" also advanced the Key Event snapshot `/get_ke_options` serves — adjacent to #239, and exactly the caveat #225 raised. The Key Events now come from the existing metadata file by default; `--refresh-metadata` opts into moving the snapshot.
  - Adds the **`ke-corpus`** and **`ke-corpus-refresh`** Makefile targets #225 asked for. A script with no target is effectively undiscoverable when `data/` is gitignored and the artifacts live only on the deployment mount.
  - The issue also reported that the script wrote the title-only file as a `shutil.copy2` of the with-description set. **That is no longer true** — it computes both channels separately, and only the backward-compat `ke_embeddings.npz` is a copy. A test now pins that, since the copy was the cause of #209.
...
- **Documentation and landing page no longer describe a WikiPathways-only tool.** The name changed when the second and third resources landed, but a good deal of the prose did not. The landing headline and page title read "Connecting Key Events to Molecular Pathways", which is wrong for Gene Ontology — GO supplies biological processes and molecular functions, not pathways. The in-app Getting Started opened by saying the app "helps researchers map Key Events from AOP-Wiki to WikiPathways", and the User Guide presented WikiPathways as Step 2 with GO and Reactome as "Step 2 (alternative)", which encodes a hierarchy the tool does not have.
  - Landing headline and `<title>` are now "Connecting Key Events to Molecular Pathways and Processes", matching the phrasing the README already used. The subtitle and the "How it works" step name what each resource contributes rather than listing all three as interchangeable.
  - Getting Started and the User Guide now state the distinction explicitly: WikiPathways and Reactome give curated pathway models, so a mapping says *this Key Event is realised by this pathway*; GO gives processes, so a mapping names *the process the Key Event is* — usually the better fit for a Key Event that describes a single event rather than a mechanism with a diagram. The three are presented as peers, with Step 2a/2b/2c replacing "Step 2" plus two alternatives.
  - The README endpoint tables omitted every Reactome route and the whole public `/api/v1` surface. Both added, the latter with a note that `per_page` is clamped at 200 and that consumers must follow `pagination.total_pages` — the trap behind #242.

### Fixed

- **Explore showed only the first 200 mappings, so the rest could not be curated (#242).** Both Explore tables requested a single page of `per_page=200` — the API's clamp, not a value the client can raise — and discarded the `pagination` envelope that reports `total`, `total_pages` and `next`. DataTables was then handed the truncated array and paginated *within* it at 25 rows a page, so the table looked complete and internally consistent while holding a subset. With 252 approved KE→WikiPathways mappings, 52 were absent with no control, no notice and no way to reach them.
  - This is a curation gap, not a display limit. "Propose Change" is bound to a row present in the table (`wpTable.row(...).data()`) and is the only route to revising or deleting an approved mapping, so a hidden row cannot be corrected, re-tiered or retired through the interface at all. During the EFSA D3.2 campaign four deletion proposals had to be posted to `/submit_proposal` by hand, and ten of eighteen pending confidence corrections were unreachable.
  - The affected rows were not a random tail: sorting is by last-updated, so the hidden remainder was the **least** recently touched mappings — the legacy rows most likely to need correction and the ones an audit is most likely to target.
  - Both tables now walk `pagination.total_pages` and concatenate. Paging stays client-side because DataTables owns sort, search and CSV/Excel/PDF export here and all three need the whole set; a comment records that the next step if this stops being affordable is server-side paging with sort and search pushed into the API, not a larger single page. A page-count backstop prevents a malformed response from looping.
  - **The Reactome table had the identical bug** and was fixed with it. It was not yet biting only because that table holds 13 rows; it would have failed the same way at 201.
  - A table that is nonetheless short now says so, rather than silently disagreeing with the database — that silence is what let an audit run against the UI report the missing rows as "not present".
  - Six regression tests. The API half seeds `MAX_PER_PAGE + 1` mappings and walks the pages, asserting every row appears exactly once, because the failure mode is that everything looks correct below the threshold and breaks quietly above it — no existing test crossed it. One test pins the clamp itself, ruling out "just request a bigger page" as a fix that would appear to work against a small corpus.

- **"Publish to Zenodo" returned 503 on every click.** #191 moved the production token from a service environment variable to a Docker secret at `/run/secrets/zenodo_api_token`, and updated the exports page render and `zenodo_publish` to go through `resolve_zenodo_token` — which checks `<VAR>_FILE`, then the secret path, then the env var. The POST route kept a raw `os.environ.get("ZENODO_API_TOKEN")`. The two then disagreed on exactly the deployed configuration: `resolve_zenodo_token` found the secret so the page rendered the button **enabled**, while the route's own check saw no env var and rejected with `503 ZENODO_API_TOKEN not configured`. Publishing was impossible from the admin UI, while `scripts/publish_zenodo.py` kept working because only the route held the stale check. The route now uses the same resolver, and its message names all three accepted sources rather than only the env var. Four tests pin the route and the button state to one source of truth; the happy-path test stubs `zenodo_publish` and the assembly helpers, since an unstubbed run would mint a real, permanent deposit under the production concept DOI.

- **WikiPathways mappings recorded no `suggestion_score` (#240).** The field is the only machine-generated provenance recording *how* a pathway was found — whether the curator took the ranker's top suggestion, picked something it scored poorly, or bypassed it via search — and it was null for every one of the 252 WP mappings in production while GO and Reactome carried theirs. Two independent gaps, either one sufficient to lose the value:
  - **The client never sent it.** `handleFormSubmission()` builds the `/submit` payload as an object literal rather than serialising `#mapping-form`, so the hidden `#suggestion_score` input — set by `selectSuggestedPathway()` from the clicked card's `data-score`, and deliberately cleared by the search and browse-panel handlers — was silently dropped on the way to the wire. The server has always read and persisted the field exactly as the GO and Reactome handlers do. Fixed by adding the key, using `|| undefined` so jQuery omits it entirely for a manual pick, matching the `step1`–`step4` convention two lines below and preserving the server's `else None` branch.
  - **The bulk-export SELECT omitted the column.** `MappingModel.get_all_mappings()` did not select `suggestion_score`, and that is the query behind the RDF and GMT exports. `rdf_exporter` guards on `if row.get("suggestion_score") is not None`, so a missing column produced valid Turtle with no `suggestionScore` triple rather than an error — meaning even a correctly stored score could never have reached the published data. `GoMappingModel` and `ReactomeMappingModel` already selected it; the paginated query behind `/api/v1` always did too, which is why the asymmetry was visible in the API payload but not attributable from it. The column is appended at the end of the SELECT per the positional-append rule the surrounding comments set out; every consumer reads by key, and the CSV export names its `fieldnames` explicitly.
  - **The existing rows stay null and are not backfilled.** The score depended on the corpus and embedding artifacts as they stood at submission time, and both have since changed — a recomputed value would misrepresent what the curator actually saw.
  - Four regression tests: one scoped to the `handleFormSubmission` object literal (a whole-file search for the string would be satisfied by the GO and Reactome payloads), one on the export query, and two HTTP round-trips asserting that a scored submission survives submit → approve → export while a manual pick stays null.

- **Approving a GO proposal discarded the curator's three dimension scores, on both approval paths.** `/submit_go_mapping` stores `connection_score`, `specificity_score` and `evidence_score` in `ke_go_proposals.proposed_*_score`, and the mapper form posts all three. Neither approval path carried them onto the mapping. Bulk-approve hardcoded `None` for each with `assessment_version = "v1"`, citing D-15 — but D-15 rules out re-scoring *as the admin*, which the bulk path cannot collect anyway, not the scores the submitter already recorded. Single-approve looked like the careful alternative and was not: `_singleApprove()` in `admin_proposals.js` posts only `admin_notes` and `csrf_token`, so the route's "all three present" test never fired and it fell into the same legacy branch. The two paths were byte-identical, so approving one at a time bought nothing while costing one request per proposal against the 20/hour ceiling. Every approved GO mapping was consequently stamped `v1` with three NULL scores, and `/api/v1/go-mappings` reported the whole table as legacy and unscored. Both paths now read the stored `proposed_*_score` values and stamp `v2` when all three are present, `v1` when the proposal predates the dimension form. An admin who *does* supply a full re-score still overrides them, with confidence recomputed server-side as before. A test asserts the two paths produce identical mapping rows.

- **Bulk-approve accepted revision and deletion proposals and silently duplicated the mapping.** The pre-flight checked only that a proposal existed and was pending, while `_approve_on_conn` unconditionally `INSERT`s into `ke_go_mappings`. A change proposal (`mapping_id` set) or a deletion (`proposed_delete`) selected in the admin queue would therefore have created a second mapping row for the pair instead of updating or removing the first — and the template renders a checkbox for every pending proposal regardless of kind, so `#selectAll` on a mixed queue was enough to trigger it. Non-new-pair proposals are now rejected in the pre-flight with `"not a new-pair proposal"` and must go through the single-approve route, which handles all three kinds. The WikiPathways and Reactome bulk paths have the same shape and are not covered by this change.

- **The directional-label filter excluded 88 Biological Process terms that encode no direction.** `\bactivated\b` and `\binhibited\b` sat alongside `positive regulation of` and friends in the shared lexicon (#193), which is applied both when building the corpus and on every search result. Measured against all 24,129 BP labels, `\bactivated\b` matched 88 terms and **not one was directional**: 62 of the form `<ligand>-activated <receptor> signaling pathway` (`TRAIL-`, `VEGF-`, `CXCL12-`, `abscisic acid-`), plus cell-state qualifiers like `activated T cell proliferation` and `activated CD4-positive, alpha-beta T cell apoptotic process`, plus `peroxisome proliferator activated receptor signaling pathway` — that is, whole families of receptor-MIE terms, hidden from curators by a pattern intended to catch signed regulation. As a past participle the word names a mechanism or a cell state; it does not sign the term. `\binhibited\b` matched nothing at all and carried the same defect. Both are removed from the lexicon in `src/utils/text.py` and its copy in `precompute_go_hierarchy.py`. Genuinely signed variants such as `positive regulation of activated T cell proliferation` were already caught by the regulation patterns and still are.

- **The GMT export cache ignored every change to the mapping tables for the rest of the calendar day (#212).** The files under `static/exports/` were named `KE-{TYPE}_{YYYY-MM-DD}_{tier}.gmt` and regenerated only when absent, so approving, editing, rejecting or deleting a mapping had no effect on them until midnight while `/api/v1/*-mappings` reflected it at once — two public surfaces of the same database disagreeing silently for up to 24 hours. It bites hardest on the molAOP Analyser, which reads its GO and Reactome gene sets from these files but its WikiPathways mappings from the REST API, so a run made mid-curation mixes today's WP with yesterday's GO and Reactome and reports all three as current. `_get_or_generate_gmt` now decides staleness from a revision fingerprint recorded in a `.gmt.rev` sidecar, the mechanism `/exports/rdf/*` received in #211 and which never reached the GMT path — the one downstream consumers actually read. Fingerprinting the source rather than hooking the write paths covers approval, rejection, admin edit, deletion and accepted change proposals alike, including write paths added later, because it asks the table what it holds instead of trusting callers to invalidate. An unknown revision counts as stale, so an unavailable model costs a regeneration rather than pinning a stale file.
  - **`MappingCountsMixin.revision()` was not strong enough to reuse as it stood.** `COUNT(*)` paired with `MAX(updated_at)` misses real edits on the live data: the mapping tables hold `updated_at` in two formats side by side, ISO `2026-07-22T14:20:46` from the application layer and `2026-07-22 19:45:08` from SQLite `CURRENT_TIMESTAMP`, and `'T'` sorts after `' '`, so `MAX()` parks on an older ISO row while newer rows are written underneath it. It also cannot see a delete-plus-insert or a column corrected by hand. It now hashes the rows, which on these hundred-row tables costs less than the regeneration a false "unchanged" answer wrongly skips. The Turtle exports inherit the improvement.
  - **The fingerprint covers the Reactome corpus under `data/`, and now genuinely covers the GO corpora too.** `data/` is a bind mount, so a corpus refresh lands without an image rebuild. Reactome is read from disk on every call, so fingerprinting its file was enough. GO is not: `generate_ke_go_gmt` resolves through `src/services/go_annotation_index`, whose module-level cache was populated once per process and never invalidated. The corpus fingerprint therefore triggered a regeneration that reproduced the *old* gene sets and then stamped the *new* corpus fingerprint on them — a sidecar asserting a corpus version the file did not contain, which neither a gunicorn worker recycle nor a redeploy would heal, because the file then looks current. Demonstrated with a sentinel gene added to `GO:0006979` in `data/go_bp_gene_annotations_propagated.json`: fingerprint changed, served gene set unchanged on the second and third request. The cache entries are now keyed on the size and mtime of the files they were built from, so they reload when the corpus moves and are still served from memory when it has not. Every consumer benefits, not only the export path — `/api/ke-genes` and the GO suggestion service read the same index.
  - **The served GMT now says which mapping state produced it.** The date-stamped filename used to be a content identifier by accident: the file was written once per day, so `KE-GO_2026-07-22_All.gmt` named one specific export and every download that day returned identical bytes. A cache that correctly follows the mapping table removes that property, and downstream analyses record exactly these filenames as their reference state. Three mechanisms replace it: a 16-hex revision segment in the filename (`KE-GO_2026-07-22_All_r58773c1c3923fba7.gmt`), an `X-Export-Revision` response header, and a comment block at the head of the file naming the resource, the export revision, the full source fingerprint, the confidence filter and the generation time. GMT has no header of its own, so provenance goes in comment lines; every line starts with `#` and contains no tab, which makes it a single field and therefore invisible to a parser that requires a descriptor, a title and at least one gene. Verified by running the molAOP Analyser's own `parse_gmt_reference_sets` and `parse_gmt_pathway_gene_map` over a generated export — both `continue` on `len(fields) < 3` before their ID regexes are reached — and the Analyser hands gseapy an already-parsed dict, never the raw file. `/admin/exports/regenerate` writes the same block. Superseded revisions of the same export are pruned once they are an hour old, so the directory holds a working set rather than one file per approval.
- **`/admin/exports/regenerate` deleted the Reactome exports and never wrote them back (#227).** The handler `rmtree`'d the whole export directory and rebuilt only KE-WP and KE-GO, so the Reactome GMT and Turtle were removed by the one action an admin has for "rebuild everything" — the dashboard lists three resources and the rebuild button produced two. It now writes the Reactome arm too, and takes its cache directory from the same `EXPORT_CACHE_DIR` constant the public download routes use rather than a second copy of the `"static/exports"` literal, which had also made the handler untestable without `rmtree`ing the repository's own `static/exports/`.
